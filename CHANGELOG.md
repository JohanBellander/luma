...
### Added
- **Watch Command (LUMA-132)** – Prototype `luma watch <file>` adds on-change revalidation with 500ms debounce. Monitors a single scaffold JSON and re-runs ingest → layout → keyboard → flow (optional score with `--score`). Supports auto pattern selection (unless `--no-auto`), JSON newline-delimited event output via `--json`, explicit viewports via `--viewports`, single-run mode via `--once`, and delta summaries (issue additions/resolutions, unreachable count changes, MUST failure state, pattern set changes, score deltas). Artifacts overwrite in selected run folder for iterative inspection. Out of scope: multi-file project watch, advanced scoring breakdown, layout diffing.
- **Init Template Option (LUMA-134)** – Added `--template <crm|dashboard|ecommerce>` to `luma init` for copying predefined scaffold templates (`<name>.scaffold.json`) into the working directory. Templates include representative Form and Table nodes and validate with `luma ingest` immediately. Safe re-run skips existing destination. Tests added to `init.command.test.ts`.
